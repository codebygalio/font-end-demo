<template>
  <div class='sub-sort'>
    <div class="sort">
      <a :class="{active:sortParams.sortField===undefined}" @click="changeSort(undefined)" href="javascript:;">默认排序</a>
    ...
    <div class="check">
      <ClCheckbox @change="changeCheck" v-model="sortParams.inventory">仅显示有货商品</ClCheckbox>
      <ClCheckbox @change="changeCheck" v-model="sortParams.onlyDiscount">仅显示特惠商品</ClCheckbox>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  setup (props, { emit }) {
    const sortParams = reactive({
      inventory: false,
      onlyDiscount: false,
      sortField: undefined, // publishTime,orderNum,price,evaluateNum
      sortMethod: undefined // asc为正序，desc为倒序，默认为desc
    })
    const changeSort = (sortField) => {
      if (sortField === 'price') {
        sortParams.sortField = sortField
        if (sortParams.sortMethod === undefined) {
