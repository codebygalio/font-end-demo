...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      const {data} = await getPersonalDetail(this.userId) // 获取个人基本信息
       this.formData = data
    },
    async getJobDetail() {
      const {data:userInfo} = await getUserDetailById(this.userId)
      const {data:jobInfo} = await getJobDetail(this.userId) // 获取个人基本信息
