...
import { getPersonalDetail, saveUserDetailById, updatePersonal } from '@/api/employees.js'
export default {
    data(){
        return {
    ...
    methods:{
        async getUserInfo(){
          const {data} =   await getUserAvatar(this.userId)
          this.userInfo = data
       ...
       async saveUser(){
           try{
            const data =  await saveUserDetailById(this.userInfo)
           this.$message({
